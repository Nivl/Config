...
| `internal/claude/sync` | Top-level `Sync` + symlink mode + `LinkDirEntries` (per-entry symlinks for `linkedDirs`, i.e. `skills/`) + precommit hook install + last-sync-commit advance. Sub-packages: `state` (paths, git wrapper, decisions cache, atomic file writes), `prompt` (3-tier conflict resolver), `settings` (settings.json merge with set semantics for `permissions.*`), `files` (per-file + per-dir 3-way merge, used for `mergedDirs`). |
...
| `internal/symlinkfs` | `Install(source, target, now, opts)` — symlink-aware idempotent installer. Existing target that's already a symlink to `source` is a no-op; anything else gets renamed to `<target>.<YYYYMMDDHHMMSS>.bkp` and replaced. `opts.Replace` deletes the collision instead of backing it up, for targets sitting inside a directory something else scans (see `claude/sync` `linkedDirs`). Shared by `dotfiles` and both `claude/sync` link paths. |
...
- **Otherwise**: `internal/claude/sync/files` copies + 3-way merges. Base = repo content at the SHA in `<RepoDir>/.sync-state/last-sync-commit`. Local = `~/.claude/<file>`. Remote = current repo HEAD. Only true conflicts (both sides diverged differently from base) prompt the user. The one exception is `skills/`, which is linked rather than merged. See below.

Copy mode splits the curated directories into two lists in `sync.go`, and the split is the thing to keep straight when adding a directory to the curated set:

- `linkedDirs` (currently just `skills`) goes through `internal/claude/sync/linkdir.go` (`LinkDirEntries`). Each **top-level entry** of `<RepoDir>/skills` becomes its own symlink at `~/.claude/skills/<entry>`, so editing a skill in the repo is immediately live with no sync step. Per-entry rather than whole-dir because `~/.claude/skills/` also holds entries this repo doesn't own. Those are locally authored skills and hand-made links into other repos. They are matched by name and left alone. A colliding entry is **deleted**, not backed up (`symlinkfs.InstallOpts.Replace`). Claude Code reads every subdirectory of `skills/` as a skill, so a `<name>.<timestamp>.bkp` sibling would come back as a duplicate. That is also why the transient staging entry `Replace` uses is dot-prefixed. The dropped bytes are the repo's own content at an older commit, recoverable from git. Home symlinks pointing into `<RepoDir>/skills/` whose target is gone (skill deleted upstream) get pruned. A broken link to anywhere else is the user's and stays. Nothing here can conflict, so `linkedDirs` never sets `HadSkips`.
- `mergedDirs` (`agents`, `commands`) stays on the per-file 3-way merge via `files.MergeDir`.

Symlink mode is unaffected by the split. `curatedItems()` concatenates both lists, so it still links all six items whole, which means `skills/` there is one whole-directory symlink rather than a per-entry set.

`skills/` leaving the merge pipeline has two consequences. `decisions.json` entries under `skills/` are never consulted again, which is harmless and not migrated. A leftover *real directory* from before the switch is only removed if the repo still carries that name. A stray dir is indistinguishable from a locally authored skill, so only dangling **symlinks** are pruned.
