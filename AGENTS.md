...
| `shared_config/.claude/` | Curated Claude Code config: `settings.json` + `skills/` + `agents/` + `commands/` + `hooks/`. Synced into `~/.claude/` per the [Claude Code config sync](#claude-code-config-sync) flow. Runtime subdirs (`projects/`, `sessions/`, `plugins/`, …) are gitignored. Permissions are mutated via `melvin-config claude perms {allow\|ask\|deny} {add\|remove} [--bash X] [--read X] [--fetch X] [--skill X] [--force] [--dry-run]`; non-git `--bash` values land in `settings.json` (raw + PATH-resolved variants), git values like `git show *` route to `hooks/git-safe-subcommands.py` as prefix tuples instead. The hook then evaluates each git command against `DENY_PREFIXES` → `ASK_PREFIXES` → `ALLOW_PREFIXES` in that precedence order. |
...
| `internal/claude/perms` | Mutates `permissions.{allow,ask,deny}` in `shared_config/.claude/settings.json` AND the three `*_PREFIXES` tables in `shared_config/.claude/hooks/git-safe-subcommands.py` for the `melvin-config claude perms` command. `Apply(s, h, target, ops, action, force)` is the top-level entrypoint: `SplitOpsByBackend` peels git+Bash ops off via `ParseGitPrefix` (canonicalises `git X *` → prefix tokens, rejects shapes without a trailing `*`); non-git ops flow through `Variants(kind, value)` and the existing `Settings` JSON round-trip (preserves unknown top-level + `permissions.*` keys via `json.RawMessage`); git prefixes flow through `GitHookFile` (regex-located block replacement, surrounding Python content preserved byte-for-byte). Cross-list conflicts are pre-flighted across BOTH backends so a mixed-batch failure leaves neither file mutated. Personal-computer-only — `cmd` enforces. |
