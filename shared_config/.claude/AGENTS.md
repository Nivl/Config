...
To add, remove, or reorder a Bash check, edit the `HOOKS` list in `dispatch-bash.py`, not `settings.json`. `settings.json` has two Bash matchers, the dispatch entry and `ask-prose-claims.py`, which runs on its own because it is an attention prompt on `git commit` rather than a security check (see "The commit-time prose check"). The other matchers (`Write`/`Edit`, `Read`, `WebFetch`, `Workflow`) still point at their hooks directly and are unaffected. Each `HOOKS` entry is `(filename, guard)`: `guard=None` runs the hook on every Bash command (an unconditional entry), and a leading-token guard mirrors the old settings.json `if`. Keep the guard on `deny-unallowlisted-host.py`: unlike the gh/cd/git hooks it does not self-guard on its leading token (it scans any command for hosts), so without the curl/wget guard it would flag unrelated commands that merely mention a URL. `tests/dispatch_bash_test.sh` verifies the dispatcher reproduces exactly what the hooks produce when run separately.
...
## No `git init` or identity change inside an existing repo

The `deny-git-init-in-repo.py` hook denies `git init` and `git config user.name` / `user.email` (local scope) when the command's cwd sits inside a repository, meaning a `.git` exists in that directory or one above it. A fixture repo belongs under `/tmp/claude`, in a directory with nothing above it, and both commands run there untouched. `--global` and `--system` config writes are not this hook's concern. The case it stops is a sub-agent whose `cd` into its fixture did not carry to the next Bash call, so the setup ran against the real repo. One review agent did that: the `git init` was a no-op, `git config user.email a@a.com` rewrote the repo's identity, and `git commit -qm init` landed a stray commit under it. The commit was reverted by hand, and the identity would have signed every later commit until someone read `git log`. The hook runs in `dispatch-bash.py` unconditionally, since it has to see `mkdir x && git init`, and it returns early on a command with no git in it. Tests in `tests/deny_git_init_in_repo_test.sh`.
...
Tool availability is not symmetric across an agent boundary, and the loss is silent in both directions. A subagent typically does not get the `Workflow` tool. A workflow agent, meaning one spawned by the `Workflow` tool's `agent()` call, does not get the `Agent` tool. Neither absence is announced. Check for the tool rather than assuming either way. The agent simply has one fewer tool than the skill it is running assumes, and each direction breaks whichever skills depend on the missing half. The first half is documented inside a skill (`work-on` Step 2 falls back to plain concurrent `Agent` calls when `Workflow` is absent). The second half is what this section is for. A sub-agent keeps its own `Agent` tool, so a skill nested inside one can still fan out. That is how `pr-review` used to launch `in-depth-review`, and it is no longer how the review skills work, for a reason the paragraph after next explains. The workflow boundary is the one that takes the tool away.

A skill that fans out has nothing to fan out with inside a workflow agent. A workflow agent has neither the `Agent` tool nor the `Workflow` tool, so `in-depth-review` cannot dispatch its eight to eleven roles, `review-and-fix` cannot dispatch its reviewers, and `pr-review` cannot dispatch its finders. Each aborts there rather than returning a review. Without that abort each would collapse to a single reader, and that single reader would report degraded coverage in the same shape a completed review uses. The caller would see a coverage field, read prose that looks like a review, and move on. That is a false negative wearing a disclaimer, which is worse than a loud failure, and it is what both guards below exist to prevent.
...
The review skills now use that same shape for their own roles, and the reason is a measured defect rather than tidiness. `in-depth-review`, `pr-review` and `review-and-fix` all drive the eleven reviewer roles through a saved workflow named `review-roles`, from the main thread, with each role an `in-depth-review-role` leaf behind one `parallel()` barrier. Before that, `pr-review` and `review-and-fix` nested `in-depth-review` inside a wrapper sub-agent, and the wrapper spawned the roles with its own `Agent` tool. A nested agent's completion notification is delivered to the session root, not to the agent that spawned it. In session `0304256d` two wrappers launched 24 roles between them, every role finished, and the wrappers received 5 and 7 of their 12 results while one ran `bash true` 111 times polling for the rest. The barrier has no notification to route. `tests/deny_review_in_workflow_test.sh` feeds the real `review-roles.js` to the hook and expects allow, so the script's wording is checked rather than trusted, and the script's own header comment says which five names it must never contain.
...
## The commit-time prose check

`hooks/ask-prose-claims.py` runs on every Bash `git commit`, as its own PreToolUse matcher rather than an entry in `dispatch-bash.py`. It reads the staged diff and denies, with the hits as the reason, when an ADDED prose line or the commit message carries one of the shapes the section above bans. Four checks. A quantifier (`nothing`, `never`, `always`, `every`, `only`, `none`, `the one`) outside backticks and URLs. A pointer, meaning a `dir/file.ext`, `file.ext:123`, `~/x/f.md` or `/abs/f.md` token in an added comment, that names no file under the repo root, the cwd, or the file's own directory. An identifier in a code-file comment (camelCase, snake_case with two or more segments, or `a.b` member access) that no code line in the staged file names, with case and underscores normalised so `payment_status` matches `paymentStatus`, since a comment naming a symbol the file lacks is a claim about a collaborator. And an added comment block in a code file over eight lines. The last two come from thirty self-inflicted fixes across five `review-and-fix` runs, thirteen of which were comments of those two shapes. Markdown is exempt from both, because a doc names symbols across files by design. Prose means a line in a markdown or text file, or a line whose first non-space characters are the comment marker for the file's extension (`#`, `//`, `/*`, `*`, `--`). Code lines are not read. The commit message is read for quantifiers when it is in the command, as `-m` values or a `-F -` heredoc body.

The deny reason lists each hit as `file:line` with the word or the pointer, capped at ten. It is a deny and not an ask because a hook's ask goes to the human, and the model sees the reason only if the human declines, so an ask cannot make the model fix its own prose. A deny's reason is the tool result. The response is to rewrite the lines and commit again, because a hit is almost always a claim that should name its set or a pointer that should resolve. The override is `PROSE_CLAIMS_OK=1 git commit ...`, for the carve-outs the section above names, an instruction, a quantifier about the function in front of you, an aside about people, or literal content the backtick rule did not catch. The hook lets that form through, and since `git` is excluded from the sandbox and that variable is not in `bash-allow-trusted`'s `safe_assignments`, the prefixed command falls to the normal permission prompt. So the override lands in front of the human, and the model has to have named each carve-out first. Measured basis: one `review-and-fix` run wrote four wrong `only` claims with the same scan in place as a reading instruction inside the skill, and another spent an iteration on a comment pointer that named the wrong paragraph. A reading instruction is skippable and a deny is not. The hook is silent on zero hits and on any failure, because a broken check must not block every commit. Tests in `tests/ask_prose_claims_test.sh`, against a fixture repo under `/tmp/claude`.

## The case-list gate on review-and-fix commits

`hooks/require-case-list.py` is a PreToolUse `Bash` matcher of its own, like the prose check. Inside a `review-and-fix` run, meaning `~/.melvin/config/logs/review-and-fix/.active-<session_id>` exists, it denies a `git commit` whose staged diff changes a code line when the run log has no `cases iter=<N> ... reaches=` line for the current iteration, where N comes from the log's own markers (`t0 iter=N`, `## Iteration N`, `stamps: iter=N`, and a bare `t0=` after the last stamps line opening the next one), since the logs write any of those. A code line is an added or removed non-blank, non-comment line in a source file whose name is not a test's, and a deleted file counts. A helper under a test directory is logic, as CLASSIFIER.md says. When the iteration's `reaches=` mentions paired state (claim, release, lock, retry, transaction, rollback, acquire), an `invariant=` line is required too, since the measured fix chains were one exit of such a region per iteration with the property never written down. Prose-, test- and comment-only diffs pass, and the check is per iteration, so the pre-commit follow-up commit passes on the list the iteration wrote. `CASE_LIST_OK=1` overrides and falls to the permission prompt. Why a hook: IMPLEMENTER.md section 0 already required the list, and on GRO-17923's second run it was written in iteration 1 and not in 2, 3 or 4, each of which fixed a correctness bug the previous iteration's fix had introduced in the same two files. That is the shape the list exists to catch, and a reading instruction decays with the iteration count where a deny does not. Outside a run the hook does nothing. Tests in `tests/require_case_list_test.sh`.

## The usage-lines hook writes the review cost

`hooks/usage-lines.py` is a PostToolUse matcher on `Workflow` and `Bash`. The `Workflow` tool now returns at launch and runs in the background, so at its PostToolUse no role has a transcript yet, and all four runs logged on 2026-09-23 got no lines from the hook. The Bash pass is the fix: while a run marker exists, each Bash call sweeps every stamped transcript that changed since the last sweep (the `.active-<session_id>.seen` file beside the marker holds the newest mtime seen) into the run log with the same dedupe, so the lines land on the orchestrator's first Bash call after the workflow's notification, the scorer's included. The rest of this paragraph describes the `Workflow` pass, which still serves `pr-review` and a standalone `in-depth-review` when a workflow runs in the foreground. When a workflow call carried `args.tag` it runs `skills/review-and-fix/usage.jq` over this session's sub-agent transcripts and keeps the `usage kind=...` lines for that tag. If `~/.melvin/config/logs/review-and-fix/.active-<session_id>` exists, it holds a run log path, and the lines are appended there, skipping any `id=` the log already has at the same turn count or higher. A logged line with fewer turns was written while that agent was still running, so it is replaced rather than skipped, which keeps one line per `id=` for the Spend total to sum over. Otherwise the lines come back as `additionalContext`, which is what `pr-review` and a standalone `in-depth-review` see. `review-and-fix` writes the marker at Step 0 and deletes it at the Final Report. Two of three logged runs skipped the skill's own instruction to run the filter and wrote an unpriced token total instead, and the hook exists so the lines exist regardless. It fires when the workflow returns, so an agent still running then, the scorer in particular, gets its line on the next tagged return in the same session, and the final iteration's on the skill's own last run of the filter. Tests in `tests/usage_lines_test.sh`, with fixture transcripts and a marker under a fixture session id.

## The prose lane in review-and-fix

A `review-and-fix` iteration that commits prose and tests and no logic hands roles 1, 5, 9 and 11 the run's own sentences to read, and the run that showed what that produces went nine iterations after its one logic-free first pass, every finding prose, 56 of them on the run's own rewrites, five sentences rewritten four to six times, at about $55 of roles an iteration plus the orchestrator. Its rootcause lines say the same thing each time: a rewrite replaced one collaborator claim with a shorter one, or one why with another. Three rules in the skill come from that run. The Step 3 ask fires after two consecutive iterations with `any_logic_change=false`, beside the older three-consecutive-majority trigger, which took until iteration 9 there because the majority dipped in two iterations while the absolute count rose. A self-inflicted prose finding is fixed by deleting the sentence or replacing it with a pointer to a symbol the file names, never a third wording, and the second finding on the same sentence deletes it, logged as `prose-locus` lines (IMPLEMENTER.md section 2). And the pre-commit check skips a batch with no logic commit, per the section below.

## The fix-implementer experiment, and why the orchestrator writes its own fixes

`agents/fix-implementer.md` exists and nothing launches it. For two runs in September 2026 `review-and-fix`'s Step 2 handed each batch of fixes to it, on the measurement that the orchestrator's fix phase, 180 turns in one iteration at 600K to 965K tokens of context each, cost more than the review fan-out. Priced at Fable's real rates that phase was about $26 an iteration, and the hand-off cut it to $9.50 while the Opus-low implementer and the check cost about $16. Cache reads are cheap on Fable ($0.25 a million against Opus 5's $0.50, and Opus 5.5 is $0.20), so a large context is not the expense it looks like, and what the implementer removed was mostly output tokens, which it then spent itself at Opus rates. Self-inflicted findings per iteration did not fall with Opus writing the fixes, at 10 and 14 against 7 to 9 before, so a cheaper tier had a quality cost with no measured upside. The writing came back to the orchestrator. What the experiment left behind is in the skill: `skills/review-and-fix/IMPLEMENTER.md` is the fix sub-steps as one checklist, `fix-precommit-check` reads every six commits as a second reader when the batch holds a logic commit and its hits land as a follow-up commit (prose and test batches are skipped, since on two runs it passed 65 of 65 of the next iteration's self-inflicted prose findings and its own follow-ups were blamed for 23 of one run's 56), and commits are grouped by class (prose in one, tests in one, logic per locus), which keeps `class` pure per commit without the 22-commit iterations that one finding per commit produced. The agent file stays so the arithmetic can be rerun if a tier's price or quality moves. Its `model` pin is whatever the last experiment set and means nothing while it is unwired.
