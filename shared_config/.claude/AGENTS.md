...
`hooks/usage-lines.py` is a PostToolUse matcher on `Workflow`. When a workflow call carried `args.tag` it runs `skills/review-and-fix/usage.jq` over this session's sub-agent transcripts and keeps the `usage kind=...` lines for that tag. If `~/.melvin/config/logs/review-and-fix/.active-<session_id>` exists, it holds a run log path, and the lines are appended there, skipping any `id=` the log already has. Otherwise the lines come back as `additionalContext`, which is what `pr-review` and a standalone `in-depth-review` see. `review-and-fix` writes the marker at Step 0 and deletes it at the Final Report. Two of three logged runs skipped the skill's own instruction to run the filter and wrote an unpriced token total instead, and the hook exists so the lines exist regardless. It fires when the workflow returns, so an agent still running then, the scorer in particular, gets its line on the next tagged return in the same session, and the final iteration's on the skill's own last run of the filter. Tests in `tests/usage_lines_test.sh`, with fixture transcripts and a marker under a fixture session id.

## The fix-implementer writes review-and-fix's fixes

`review-and-fix`'s Step 2 hands its fixes to `agents/fix-implementer.md` in batches of up to six findings, one commit per finding, one launch per batch, with the orchestrator idle. The orchestrator keeps the judgement that needs its context, which finding, which approach, whether it reopens an earlier decision, and every run-log line. The implementer does the red test, the edit, lint and tests, the staged-diff checks and the commit, following `skills/review-and-fix/IMPLEMENTER.md`, and after the batch `fix-precommit-check` reads the batch's commits and the implementer lands the hits as one follow-up commit. It cannot launch that check itself, because a nested agent's result goes to the session root. Batching is what makes the hand-off pay: the orchestrator's seven hand-off turns run at several hundred thousand tokens of context each, and paying them once per finding cost about what the old in-context fix did, while once per batch is estimated to cut the fix phase by roughly 60 percent at the measured average of about eight findings an iteration, an estimate the implementer's usage lines will replace. Measured basis, session `43c951f8` on GRO-18343: the orchestrator's fix phase was 180 turns in iteration 1 alone, each turn re-reading 600K to 965K tokens, and at Opus rates the orchestrator's ten iterations cost at least as much as the entire review fan-out, with 63 percent of it in the fix phase. The implementer starts from a packet of a few thousand tokens. Its tier is pinned in its file, Opus at low to start, and it is forbidden the same git commands as the reviewers plus any edit outside the packet's file list. The one-writer rule is what makes a tree-editing sub-agent safe here and not for reviewers, which run concurrently.
