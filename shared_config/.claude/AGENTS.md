...
`hooks/ask-prose-claims.py` runs on every Bash `git commit`, as its own PreToolUse matcher rather than an entry in `dispatch-bash.py`. It reads the staged diff and asks, never denies, when an ADDED prose line or the commit message carries one of the shapes the section above bans. Two checks. A quantifier (`nothing`, `never`, `always`, `every`, `only`, `none`, `the one`) outside backticks and URLs. A pointer, meaning a `dir/file.ext`, `file.ext:123`, `~/x/f.md` or `/abs/f.md` token in an added comment, that names no file under the repo root, the cwd, or the file's own directory. Prose means a line in a markdown or text file, or a line whose first non-space characters are the comment marker for the file's extension (`#`, `//`, `/*`, `*`, `--`). Code lines are not read. The commit message is read for quantifiers when it is in the command, as `-m` values or a `-F -` heredoc body.

The prompt lists each hit as `file:line` with the word or the pointer, capped at ten. The right response is to amend, because a hit is almost always a claim that should name its set or a pointer that should resolve. Approving is for the carve-outs the section above names, an instruction, a quantifier about the function in front of you, an aside about people, or literal content the backtick rule did not catch. Measured basis: one `review-and-fix` run wrote four wrong `only` claims with the same scan in place as a reading instruction inside the skill, and another spent an iteration on a comment pointer that named the wrong paragraph. A reading instruction is skippable and a prompt is not. The hook is silent on zero hits and on any failure, since it is a prompt for attention rather than a gate the work depends on. Tests in `tests/ask_prose_claims_test.sh`, against a fixture repo under `/tmp/claude`.
