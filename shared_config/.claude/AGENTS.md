...
The allowlist is the `WebFetch(domain:…)` allow rules plus `sandbox.network.allowedDomains`; anything else is blocked by the sandbox at the kernel. The fix is `melvin-config claude perms allow add --fetch <host>`, then `melvin-config claude sync` + restart (**the user** runs these — sync needs to escape the sandbox); re-run once the host is allowlisted.
