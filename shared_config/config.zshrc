...
# NPM
export YARN_CACHE_FOLDER="$SDKS_ROOT/yarn/cache"
mkdir -p "$YARN_CACHE_FOLDER"
export NPM_CONFIG_CACHE="$SDKS_ROOT/npm"
...
alias lsd='lsd --config-file="$HOME/.melvin/config/shared_config/lsd.yaml"'
alias ls='lsd -hF'
alias lt='lsd --tree'
alias ll='lsd -l'
alias lla='lsd -la'
...
function wt() {
  ...
  local remote_url
  remote_url=$(git -C "$project_dir" config --get remote.origin.url) || return 1
  ...
  if [ -n "$existing_worktree_path" ]; then
    ...
    # Refuse to clobber a stale directory at worktree_path. Without this
    # guard, `git worktree add` would fail with an opaque "already
    # exists" error and we'd still cd into the bogus path below.
    if [ -e "$worktree_path" ]; then
      echo "wt: '$worktree_path' already exists but is not a registered worktree. Remove it or pick a different feature name." >&2
      return 1
    fi
    ...
    if git -C "$project_dir" show-ref --verify --quiet "refs/heads/$feature_name"; then
      git -C "$project_dir" worktree add "$worktree_path" "$feature_name" || return 1
    else
