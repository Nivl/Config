...
#   git commit with an unnamed quantifier in staged prose      -> ask
#   git commit with an unresolved pointer in a staged comment  -> ask
#   quantifier in the -m / heredoc commit message              -> ask
...
# ---- Quantifier in staged prose (markdown) -> ask ----
stage docs/notes.md 'This helper is the only reader of the flag.'
assert_eq "ask_md_quantifier" "ask" "$(decision 'git commit -m "docs"')"
assert_contains "reason_names_file_line" "docs/notes.md:2: \`only\`" "$(reason 'git commit -m "docs"')"
unstage_all

# ---- Quantifier in a TS comment -> ask; in TS code -> silent ----
stage src/a.ts '// every caller checks the flag first'
assert_eq "ask_ts_comment" "ask" "$(decision 'git commit -m x')"
unstage_all
...
# ---- Quantifier in the commit message -> ask (dash-m and heredoc) ----
assert_eq "ask_message_m" "ask" "$(decision 'git commit -m "this is the only fix"')"
printf -v HEREDOC "git commit -q -F - <<'EOF'\nfix: thing\n\nNothing else reads it.\nEOF"
assert_eq "ask_message_heredoc" "ask" "$(decision "$HEREDOC")"
...
assert_eq "ask_pointer_missing" "ask" "$(decision 'git commit -m x')"
...
assert_eq "ask_pointer_abs_missing" "ask" "$(decision 'git commit -m x')"
unstage_all
